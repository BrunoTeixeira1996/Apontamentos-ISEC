...
int _tmain(int argc, TCHAR* argv[]) {
	...
	TCHAR caminho_chave[TAM] = TEXT("SOFTWARE\\Aula\\");
...
#endif
	//Pede o nome da chave
	_tprintf(TEXT("\nNome da chave: "));
	fflush(stdin);
	_fgetts(chave_nome, TAM, stdin);
	chave_nome[_tcslen(chave_nome) - 1] = '\0';

	_tcsncat_s(caminho_chave, TAM,chave_nome, TAM - _tcslen(caminho_chave) - 1);
	_tprintf(TEXT("caminho: %s\n"), caminho_chave);
