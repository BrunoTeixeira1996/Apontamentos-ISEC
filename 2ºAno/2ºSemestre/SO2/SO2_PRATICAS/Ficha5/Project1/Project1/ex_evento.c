...
DWORD WINAPI ThreadMultiplos(LPVOID param) {
    ...
    //Todas as threads sao lançadas, mas todas vao bloquear nesta linha até que um evento seja assinalado e elas possam desbloquear
...
int _tmain(int argc, TCHAR* argv[])
{
    ...
    if (hEventGlobal == NULL) {
        _tprintf(TEXT("Erro a criar evento\n"));
        return 1;
    }

    //inicializar a critical section e o spin count
    ...
    for (i = 0; i < numthreads; i++) {
        ...
        tdados[i].cs = &csGlobal;
        tdados[i].hEvent = hEventGlobal;

        //Aqui as threads vao ser criadas e automaticamente executadas
